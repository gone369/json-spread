...
describe('Unspread', function(){
  describe('Input', function(){
    it('should throw Error when input is not Object or Array' ,function () {
      expect(function(){jsonSpread.unspread("string")}).to.throw(TypeError);
      expect(function(){jsonSpread.unspread(1)}).to.throw(TypeError);
    });
    it('should not throw Error when input is Object or Array' ,function () {
      expect(function(){jsonSpread.unspread({})}).to.not.throw(TypeError);
      expect(function(){jsonSpread.unspread([])}).to.not.throw(TypeError);
    });
  });
  ...
  describe('Output', function(){
    describe("Multiple Array", function(){
      it('input should be equal to output json' , function(){ 
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(multipleArrOutput)),JSON.stringify([ multipleArrInput ]));
      })
    })
    describe("Nested Array", function(){
      it('input should be equal to output json' , function(){ 
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(nestedArrOutput)),JSON.stringify([ nestedArrInput ]));
      })
    })
    describe("Nested Object", function(){
      it('input should be equal to output json' , function(){ 
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(nestedObjOutput)),JSON.stringify([ nestedObjInput ]));
      })
    })
    describe("Nested Array Objects", function(){
      it('input should be equal to output json' , function(){ 
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(nestedArrObjOutput),"a.index"),JSON.stringify([ nestedArrObjInput ]));
      })
    })
    describe("Nested Object Arrays", function(){
      it('input should be equal to output json' , function(){ 
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(nestedObjArrOutput)),JSON.stringify([ nestedObjArrInput ]));
      })
    })
    describe("Complex Object", function(){
      it('input should be equal to output json' , function(){
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(complexObjOutput)),JSON.stringify([ complexObjInput ]));
      })
    })
    describe("Complex Array", function(){
      it('input should be equal to output json' , function(){
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(complexArrOutput)),JSON.stringify(complexArrInput));
      })
    })
    describe("Real Life Example", function(){
      it('input should be equal to output json' , function(){
        assert.strictEqual(JSON.stringify(jsonSpread.unspread(reallifeOutput)),JSON.stringify(reallifeInput));
      })
    })
  })
  ...
  describe('Options', function(){
    describe("Remove Empty Arrays" , function(){
      it("should remove empty array fields", function(){
        assert.strictEqual(JSON.stringify(jsonSpread.unspread({a: 1, b:[]},{removeEmptyArray: true})),JSON.stringify([{a: 1}]));
      })
    })
    describe("Empty Arrays Value" , function(){
      it("should replace empty array fields with 'EMPTY'", function(){
        assert.strictEqual(JSON.stringify(jsonSpread.unspread({a: 1, b:[]},{emptyValue: "EMPTY"})),JSON.stringify([{a: 1, b: "EMPTY"}]));
      })
    })
    describe("Delimiter" , function(){
      it("should use '*' as delimiters", function(){
        assert.strictEqual(JSON.stringify(jsonSpread.unspread({"a*b": 1},{delimiter: "*"})),JSON.stringify([{a: { b: 1 }}]));
      })
    })
  })
})
